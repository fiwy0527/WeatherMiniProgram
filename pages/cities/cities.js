...
Page({
  ...
  citySelected: function (e) {
    ...
    wx.redirectTo({
      url: `../index/index?city=${cityNameTemp}` ,
    })
  },
  bindSearchInput: function (e) {
    ...
    for (var i = 0; i < cities.length; i++) {
      if (cities[i].cityName.indexOf(inputVal) == 0 || cities[i].cityPY.indexOf(inputVal.toLowerCase()) == 0 || cities[i].cityPinYin.indexOf(inputVal.toLowerCase()) == 0) {
        //去除热门城市
        if (cities[i].cityPY.indexOf("#") != -1) {
          continue;
        }
        var ifHas = false;
        for (var j = 0; j < cityResultsTemp.length; j++) {
          if (cityResultsTemp[j] == cities[i]) {
            ifHas = true;
            break;
          }
        }
        if (!ifHas) {
          cityResultsTemp.push(cities[i]);
        }
      }
    }
    this.setData({
